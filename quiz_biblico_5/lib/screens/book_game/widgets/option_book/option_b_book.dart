...
class OptionBBook extends StatelessWidget {
  final VariablesGameModel book;
  final String nameBook;

  const OptionBBook({super.key, required this.book, required this.nameBook});

  @override
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        if (book.alternatives2Imagem == 'images/balloons.png') {
          enviarRanking(10);
        } else if (book.response2 == 'Você Perdeu 2 Pontos!') {
          enviarRanking(-2);
        } else if (book.response2 == 'Você Perdeu 5 Pontos!') {
          enviarRanking(-5);
        } else if (book.response2 == 'Você Perdeu 15 Pontos!') {
          enviarRanking(-15);
        } else if (book.response2 == 'Você Perdeu 10 Pontos!') {
          enviarRanking(-10);
        }
        Navigator.of(context).pushReplacementNamed(
          RoutesBooks().routes['${nameBook}AnswerB']!,
          arguments: book,
        );
      },
      child: Padding(
